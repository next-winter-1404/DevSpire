...
export default function MockPaymentView({
  ...
  const onSubmit = (data: PaymentFormData) => {
    toast.success(t("paymentInfoSaved"));
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl overflow-hidden flex flex-col md:flex-row">
        <div className="bg-blue-600 text-white md:w-1/3 p-8 flex flex-col justify-between">
          <div>
            ...
            <div className="space-y-4 text-sm">
              <div className="flex justify-between">
                <span className="text-blue-200">{t("merchantId")}:
                </span>
                <span>{data.userId}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-blue-200">{t("orderId")}:
                </span>
                <span>{data.bookingId}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-blue-200">{t("transactionId")}:
                </span>
                <span>{data.id}</span>
              </div>
              <div className="flex justify-between items-center mt-6 p-4 bg-blue-700 rounded-lg">
                <span className="text-blue-200">{t("payableAmount")}:
                </span>
                <div className="text-left">
                  <span className="text-2xl font-bold block">
                    {parseInt(data.amount).toLocaleString()}
                  </span>
                  <span className="text-xs text-blue-300">{t("toman")}
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="mt-8 text-center bg-blue-700/50 py-3 rounded-lg">
            <p className="text-sm text-blue-200 mb-1"> {t("remainingTime")}
            </p>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div>
              ...
              <input
                ...
                className={`w-full text-black px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 text-center tracking-widest text-lg ${errors.cardNumber
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  dir="ltr"
                  placeholder="***"
                  className={`w-full px-4 py-3 text-black border rounded-lg focus:ring-2 focus:ring-blue-500 text-center tracking-widest ${errors.cvv2 ? "border-red-500 bg-red-50" : "border-gray-300"
                    }`}
                  maxLength={4}
                  {...register("cvv2", {
                ...
                <div className="flex items-center gap-2" dir="ltr">
                  <div className="flex-1">
                    <input
                      type="text"
                      placeholder="سال"
                      className={`w-full px-4 py-3 border text-black rounded-lg focus:ring-2 focus:ring-blue-500 text-center ${errors.expYear
                          ? "border-red-500 bg-red-50"
                          : "border-gray-300"
                        }`}
                      maxLength={2}
...
,
                  ...
                  <div className="flex-1">
                    <input
                      type="text"
                      placeholder="ماه"
                      className={`w-full px-4 py-3 text-black border rounded-lg focus:ring-2 focus:ring-blue-500 text-center ${errors.expMonth
                          ? "border-red-500 bg-red-50"
                          : "border-gray-300"
                        }`}
                      maxLength={2}
