...
export default function ForgotPasswordStep3({ back, data }: Props) {
  ...
  const handleSubmit = async () => {
    const email =
      data?.email || localStorage.getItem("resetEmail")?.trim() || "";
    const userId = data?.userId || localStorage.getItem("resetUserId");
    const resetCode =
      data?.resetCode || localStorage.getItem("resetCode")?.trim() || "";
    ...
    try {
      ...
      const msg = err.response?.data?.message || err.message || t("resetError");
  ...
  return (
    <div className="w-full h-full flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-4 mt-4">
        <div className="w-full flex flex-col gap-3 mb-[24px] lg:mb-[40px]">
