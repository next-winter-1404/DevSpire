...
export default function RegisterStep2View({ next, back, data, updateData }: Props) {
...
useEffect(() => {
    if (verifyState?.success && !data.userId) { 
        toast.success(t("verificationSuccess") || "Email verified successfully");
                updateData({ userId: verifyState.userId }); 
...
}, [verifyState, next, t, data.userId, updateData]); 
    ...
    return (
        <div className={shabnam.className} dir={direction}>
            ...
            <div className="flex flex-col">
                <div onClick={back} className="flex items-center gap-[3px] mt-[16px] lg:mt-[32px] mb-[16px] lg:mb-[24px] cursor-pointer animate-[fadeText_0.7s_ease]">
                    <Image src="/icons/fastReservePage/Frame (2).svg" alt="back" width={22} height={22} className="dark:invert" />
                    <span className="text-[#0D3B66] dark:text-white text-[16px]">{t("back")}</span>
                </div>
                <div className="lg:hidden relative w-full h-[180px] rounded-[24px] overflow-hidden mb-[24px]">
                    <Image src="/images/fastReservePage/login.jpg" alt="login" fill className="object-cover" />
                </div>
...
<p className="text-[#1E2022] dark:text-white/70 text-[14px] lg:text-[16px] leading-[22px] lg:leading-[24px]">
    {t("description")} <strong>{data.email}</strong>
                    ...
                    <p className="text-[#0D3B66] dark:text-white text-[14px] lg:text-[16px] cursor-pointer">{t("changeEmail")}</p>
                </div>
                <form action={verifyAction} className="w-full lg:w-[552px] flex flex-col gap-[20px] lg:gap-[32px] mb-[24px] lg:mb-[32px]">
<input type="hidden" name="tempUserId" value={data.tempUserId || ""} />
                    <input type="hidden" name="verificationCode" value={otp.join("")} />
                    <div className="w-full flex justify-between gap-2" dir="ltr">
                        {otp.map((digit, i) => (
                            <input key={i} ref={inputRefs[i]} maxLength={1} type="text" inputMode="numeric" value={digit}
                                onChange={(e) => handleChange(e.target.value, i)}
                                onKeyDown={(e) => { if (e.key === "Backspace" && !otp[i] && i > 0) inputRefs[i - 1].current?.focus(); }}
                    ...
                    {verifyState.message && !verifyState.success && (
                        <p className="text-red-500 text-sm text-center">{verifyState.message}</p>
                    )}
    ...
    startTransition(() => {
        ...
        resendAction(formData);
                    ...
                    <button
                        type="submit"
                        className="w-full h-[52px] lg:h-[62px] rounded-[40px] flex justify-center items-center bg-[#0D3B66] text-white text-base transition-all duration-200 hover:bg-[#0D3B66]/80 animate-[fadeText_0.7s_ease] disabled:opacity-50"
                    >
                        {t("confirmContinue")}
                    </button>
