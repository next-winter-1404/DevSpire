...
const FilterModal = ({ setIsOpenFilterModal }: IProps) => {
  ...
  const estateTypeOptions = [
    {
      value: "created_at",
      label: locale == "en" ? "created at" : "زمان ایجاد شده",
    },
    {
      value: "updated_at",
      label: locale == "en" ? "last updated" : "زمان بروز شده",
    },
  ];
      ...
      <div className="flex flex-col gap-8 w-[563px] p-8 bg-[#FFFFFF] rounded-[24px] absolute top-40 right-124 z-30">
        <div className="flex justify-between items-center">
          ...
          <div
            ...
            className="flex items-center p-4 bg-[#F5F5F5] rounded-full"
          >
        ...
        <div className="flex gap-8">
          <CustomSelect
            ...
            defaultValue={estateTypeOptions[0].value}
            onValueChange={getEstateTypeOptions}
          />
          <CustomSelect
            options={estateStatusOptions}
            defaultValue={estateStatusOptions[0].value}
            onValueChange={getEstateStatusOptions}
          />
        </div>
        <TwoRangeSlider
          max={50000000000}
        ...
        <div className="flex gap-6 font-regular text-[16px]">
          ...
          <button className="w-full py-[13px] text-[#FFFFFF] bg-[#0D3B66] rounded-[16px] cursor-pointer">
            اعمال فیلتر
          </button>
        </div>
      </div>
