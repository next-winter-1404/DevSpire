...
import { DeleteTour } from "../services/DELETE/DeleteTour";
...
const DeleteTourModal = ({ id, handleDeleteModal }: IProps) => {
  ...
  const deleteLocationMutation = useMutation({
    ...
    onSuccess: (res) => {
      toast.success(res?.data?.message || "تور مورد نظر با موفقیت حذف شد");
  ...
  return (
    <div className="fixed inset-0 z-90">
      ...
      <div className="flex flex-col items-center gap-8 w-104 p-8 bg-[#FFFFFF] rounded-[24px] absolute top-[32%] left-[36%]">
        ...
        <p className="text-[#1E2022]">{t("sureAboutDeleteTour")}</p>
