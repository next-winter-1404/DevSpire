...
const DeleteCategoryModal = ({id, handleDeleteCatModal}: IProps) => {
    ...
    const deleteCategoryMutation = useMutation({
        mutationFn: () => DeleteCategory(id),
            onSuccess: (res) => {
            toast.success(res?.data?.message || "دسته بندی مورد نظر با موفقیت حذف شد");
    ...
    return (
        <div className="fixed inset-0 z-90">
            <div 
            onClick={() => {handleDeleteCatModal(false)}}
            ...
            <div className="flex flex-col items-center gap-8 w-104 p-8 bg-[#FFFFFF] rounded-[24px] absolute top-[32%] left-[36%]">
                ...
                <p className="text-[#1E2022]">{t("sureAboutDeleteCategory")}</p>
