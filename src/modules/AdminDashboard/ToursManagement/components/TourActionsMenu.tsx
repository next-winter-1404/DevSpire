    ...
    <>
      <td>
        ...
        ${locale === "en" ? "right-[4%]" : "left-[4%]"}`}>
          <Link href={`/dashboard/admin/locations-management/${id}`} className="flex items-center gap-2 w-full py-1 px-2 text-[#1E2022] 
      ...
      {isOpenDeleteModal && <DeleteTourModal id={id} handleDeleteModal={handleDeleteModal}/>}
    </>
  )
